...
- **Unit 2**: Logging in via an authorization provider, and using a token provided by that provider to grant access to features in your service (OAuth)
- **Unit 3**: Session-based authentication, when the server keeps track of user sessions.
- **Unit 4**: Token-based authentication, when the server issues a time sensitive token that grants access to the service's features for a set period of time.

### Authentication vs. Authorization

_Authentication_ and _authorization_ are not the same thing...

**Authentication** verifies a user's identity. Think signing in with your username/password as well as security questions. This is a system trying to verify that a user is who they claim to be.

**Authorization** Typically after a user has ben authenticated they are provided with a session token, or some other key or web token and they use that token for all actions performed after authentication to gain access to certain parts of application behavior. For example:
...
You can think of authentication as someone verifying they have purchased a ticket to a theme park at the will-call booth, and authorization is that same user being given an express line wristband and using that wristband at all theme park attractions to use the express line once let inside. Authentication verifies that someone is who they claim to be, while authorization declares what that individual can do once they have verified who they are. Authentication needs to take place before authorization can be provided.
...
The bottom line is that users prefer to use OAuth-like authorization protocols instead of creating another set of credentials to use your site.

This essentially means you hand the authorization concerns off to some other system and you use the OAuth protocol to authorize user behavior in your app using the authorization provided by an external authentication effort.
...
OAuth is an open standard that provides **client applications** access to **resources** of a service such as Google with the permission of the resources' **owner**. This means OAuth isn't in itself a singular thing, but rather a protocol that different entities agreed to adhere to so everyone can share authorization concerns securely amongst themselves without having to build some centralized system. Think of OAuth as a set of requirements that many companies adhere to so it is easier for everyone to use a sharable solution. OAuth is an **authorization** protocol.
...
This means if you choose to use OAuth with a google account, you're offloading all of your user authentication concerns to Google and using a shared protocol(OAuth2.0) to be able to use Google's authorization system inside of your own system. This means you don't have to worry about authentication, and you use a third party system for all of your authorization.

It is important to note that what we just described is actually called a [federated login flow](https://www.okta.com/identity-101/what-is-federated-identity/), and this flow is one of the more common use cases for OAuth. This does not mean that all OAuth implementation require federated login flows, nor does it mean all federated login flows require OAuth. There is a lot of nuance in the authentication and authorization space, but for the time being, making yourself aware of what OAuth is and some common ways to use it as well as what session-based and token-based authentications are will serve you very well as you begin your career. 
...
```js
// DELETE /reviews
router.delete('/reviews', reviewsCtrl.delete);
```
