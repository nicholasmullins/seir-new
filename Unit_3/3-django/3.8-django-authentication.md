...
If your Cat Collector is up and running from the last lesson, there's no reason to fetch latest from our git origin, otherwise...

<details>
<summary>
👀 Do you need to fetch latest from your code?
</summary>
<hr>
...
If you pulled updated code, it doesn't hurt to run `python3 manage.py migrate` just in case.
...
If the user is not logged in, we often refer to that user as an **anonymous user** or a **visitor**.
...
Django's built-in authentication functionality is provided by the `'django.contrib.auth'` and `'django.contrib.contenttypes'` apps included within the `INSTALLED_APPS` list in `settings.py`:
...
Protecting class-based views is slightly different, it uses what's called a mixin. A mixin is a term used in reference to a class in object oriented programming that has methods to be used by other classes without having to inherit the mixin class. This is similar to an interface with invokable methods, sometimes referred to as an abstract class, however the mixin doesn't require being inherited into the subclass like an abstract. The purpose of mixins is to help solve for potential ambiguity with `'multiple inheritance'` that occasionally plagues object oriented programming languages.

`'Multiple inheritance'` can be defined simply as a single class inheriting from more than one parent class.

In our case, we're using a mixin type class to easily inject authorization workflows into out view classes to verify that a request was made by an authenticated user, without needlessly bloating our inheritance trees and causing unhelpful complexity and runtime bloat. 

When you see terms like `'mixin'` that's a sign that your programming language is working with `'multiple inheritance'` paradigms. 

As usual, we'll need to import login_required mixin to your views.py:
